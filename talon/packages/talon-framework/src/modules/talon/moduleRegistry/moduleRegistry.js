...
export class ModuleRegistry {
    ...
    evaluateModuleDependency(dependency, moduleExports) {
        ...
        // Handle Salesforce scoped case
        if (dependency.startsWith('@salesforce/')) {
            const [moduleName, component] = dependency.replace(/^@salesforce\//, '').split('/');
            this.registry[dependency] = this.evaluateSalesforceModuleDependency(moduleName, component);
        ...
        assert(this.registry[dependency], `Cannot resolve dependency '${dependency}'`);
        return this.registry[dependency];
    }

    /**
     * Evaluate scoped Salesforce dependency from its module name and component.
     *
     * eg '@salesforce/apex/LoginFormController.login' or '@salesforce/cssvars/customProperties'
     *
     * @param {string} moduleName - A Salesforce scoped module
     * @param {string} component - The component name
     */
    evaluateSalesforceModuleDependency(moduleName, component) {
        switch (moduleName) {
            case 'apex':
                return getApexInvoker(component);
            case 'cssvars':
                return this.evaluateSalesforceCssVars(component);
            default:
                throw new Error(`Cannot resolve module '${moduleName}'`);
        }
    }

    evaluateSalesforceCssVars(cssvar) {
        if (cssvar === 'customProperties') {
            return style;
        }
        throw new Error(`Cannot resolve cssvar '${cssvar}'`);
    }
    ...
    evaluateUnscopedModuleDependency(moduleName, component) {
        switch (moduleName) {
            case 'lightning': {
                // Try interop case
                const nameInterop = ['interop', component].join('/');
                assert(this.registry[nameInterop], `Cannot resolve dependency ${nameInterop}`);
                return this.registry[nameInterop];
            }
            default:
                throw new Error(`Cannot resolve module '${moduleName}'`);
        }
...
export const { addModule, addModules, getModule, getTemplate, getModuleIfPresent, hasModule, setResourceUids } = instance;

export default { addModule, addModules, getModule, getTemplate, getModuleIfPresent, hasModule, setResourceUids };
