...
var Talon = (function (__callKey0,__setKey,__callKey1,__callKey2,__concat,_slicedToArray,_assertThisInitialized,_get2,_possibleConstructorReturn,_getPrototypeOf,_inherits,_defineProperty,_classCallCheck,_createClass,_instanceof,_typeof,__inKey,__callKey3,__callKey4,_toArray,_regeneratorRuntime,_asyncToGenerator,__deleteKey,__hasOwnProperty,_toConsumableArray,_objectDestructuringEmpty,__setKeyPostfixDecrement,__setKeyPostfixIncrement) {
    'use strict';
    ...
    function isLWC(element) {
      return _instanceof(element, Element) && tagNameGetter.call(element).indexOf('-') !== -1;
    }
    ...
    function getFormattedComponentStack(elm) {
      ...
      do {
        if (isLWC(currentElement)) {
          ArrayPush.call(componentStack, "".concat(indentation, "<").concat(StringToLowerCase.call(tagNameGetter.call(currentElement)), ">"));
    ...
    var assert$1 = {
      ...
      logError: function logError(message, elm) {
        var msg = "[LWC error]: ".concat(message);

        if (elm) {
          msg = "".concat(msg, "\n").concat(getFormattedComponentStack(elm));
        }
      ...
      logWarning: function logWarning(message, elm) {
        var msg = "[LWC warning]: ".concat(message);
    ...
    function notifyMutation(target, key) {
      {
        assert$1.invariant(!isRendering, "Mutating property ".concat(toString(key), " of ").concat(toString(target), " is not allowed during the rendering life-cycle of ").concat(vmBeingRendered, "."));
      }
      ...
      if (!isUndefined(reactiveRecord)) {
        ...
        if (value) {
          ...
          for (var _i3 = 0; _i3 < _len2; _i3 += 1) {
            ...
            {
              assert$1.isTrue(vm && "cmpRoot" in vm, "".concat(vm, " is not a vm."));
            }
    ...
    function createTrackedPropertyDescriptor(Ctor, key, enumerable) {
      return {
        ...
        set: function set(newValue) {
          ...
          {
            assert$1.isTrue(vm && "cmpRoot" in vm, "".concat(vm, " is not a vm."));
            assert$1.invariant(!isRendering, "".concat(vmBeingRendered, ".render() method has side effects on the state of ").concat(vm, ".").concat(String(key)));
          }
    ...
    function getPublicMethodsHash(target, publicMethods) {
      ...
      return publicMethods.reduce(function (methodsHash, methodName) {
        {
          assert$1.isTrue(isFunction(target.prototype[methodName]), "Component \"".concat(target.name, "\" should have a method `").concat(methodName, "` instead of ").concat(target.prototype[methodName], "."));
        }
    ...
    function api(target, propName, descriptor) {
      ...
      {
        assert$1.invariant(!descriptor || isFunction(descriptor.get) || isFunction(descriptor.set), "Invalid property ".concat(toString(propName), " definition in ").concat(target, ", it cannot be a prototype definition if it is a public property. Instead use the constructor to define it."));

        if (isObject(descriptor) && isFunction(descriptor.set)) {
          assert$1.isTrue(isObject(descriptor) && isFunction(descriptor.get), "Missing getter for property ".concat(toString(propName), " decorated with @api in ").concat(target, ". You cannot have a setter without the corresponding getter."));
        }
      }
      ...
      return {
        get: function get() {
          ...
          if (isBeingConstructed(vm)) {
            {
              assert$1.logError("".concat(vm, " constructor should not read the value of property \"").concat(toString(key), "\". The owner component has not yet set the value. Instead use the constructor to set default values for properties."), vm.elm);
            }
        ...
        set: function set(newValue) {
          ...
          {
            assert$1.isTrue(vm && "cmpRoot" in vm, "".concat(vm, " is not a vm."));
            assert$1.invariant(!isRendering, "".concat(vmBeingRendered, ".render() method has side effects on the state of ").concat(vm, ".").concat(toString(key)));
          }
          ...
          if (isTrue(vm.isRoot) || isBeingConstructed(vm)) {
...

            {
              ...
              if (!isObservable && !isNull(newValue) && isObject(newValue)) {
                assert$1.logWarning("Assigning a non-reactive value ".concat(newValue, " to member property ").concat(toString(key), " of ").concat(vm, " is not common because mutations on that value cannot be observed."), vm.elm);
              }
            }
          }

          {
            if (vmBeingUpdated !== vm) {
              ...
              assert$1.logWarning("If property ".concat(toString(key), " decorated with @api in ").concat(vm, " is used in the template, the value ").concat(toString(newValue), " set manually may be overridden by the template, consider binding the property only in the template."), vm.elm);
            }
          }
    ...
    function createPublicAccessorDescriptor(Ctor, key, descriptor) {
      ...
      if (!isFunction(_get)) {
        {
          assert$1.fail("Invalid attempt to create public property descriptor ".concat(toString(key), " in ").concat(Ctor, ". It is missing the getter declaration with @api get ").concat(toString(key), "() {} syntax."));
        }
      ...
      return {
        ...
        set: function set(newValue) {
          ...
          if (_set) {
            ...
            assert$1.fail("Invalid attempt to set a new value for property ".concat(toString(key), " of ").concat(vm, " that does not has a setter decorated with @api."));
          }
        },
        enumerable: enumerable
      };
    ...
    */
    ...
    function getIE11FakeShadowRootPlaceholder(host) {
      var shadowRoot = getInternalField(host, ShadowRootKey); // @ts-ignore this $$placeholder$$ is not a security issue because you must
    ...
    var getInternalChildNodes = isFalse(hasNativeSymbolsSupport$1) ? function (node) {
    ...
    function initSlotObserver() {
      return new MutationObserver(function (mutations) {
        ...
        forEach.call(mutations, function (mutation) {
          {
            assert$1.isTrue(mutation.type === 'childList', "Invalid mutation type: ".concat(mutation.type, ". This mutation handler for slots should only handle \"childList\" mutations."));
          }
    ...
    function isNodeOwnedBy(owner, node) {
      {
        assert$1.invariant(_instanceof(owner, HTMLElement), "isNodeOwnedBy() should be called with an element as the first argument instead of ".concat(owner));
        assert$1.invariant(_instanceof(node, Node), "isNodeOwnedBy() should be called with a node as the second argument instead of ".concat(node));
        assert$1.isTrue(_compareDocumentPosition.call(node, owner) & DOCUMENT_POSITION_CONTAINS, "isNodeOwnedBy() should never be called with a node that is not a child node of ".concat(owner));
      }
    ...
    function isNodeSlotted(host, node) {
      {
        assert$1.invariant(_instanceof(host, HTMLElement), "isNodeSlotted() should be called with a host as the first argument instead of ".concat(host));
        assert$1.invariant(_instanceof(node, Node), "isNodeSlotted() should be called with a node as the second argument instead of ".concat(node));
        assert$1.isTrue(_compareDocumentPosition.call(node, host) & DOCUMENT_POSITION_CONTAINS, "isNodeSlotted() should never be called with a node that is not a child node of ".concat(host));
      }
    ...
    function attachDOMListener(elm, type, wrappedListener) {
      ...
      if (cmpEventHandlers.length === 0) {
        ...
        if (ArrayIndexOf.call(cmpEventHandlers, wrappedListener) !== -1) {
          assert$1.logWarning("".concat(toString(elm), " has duplicate listener for event \"").concat(type, "\". Instead add the event listener in the connectedCallback() hook."), elm);
        }
      }
    ...
    function addCustomElementEventListener(elm, type, listener, options) {
      {
        assert$1.invariant(isFunction(listener), "Invalid second argument for this.addEventListener() in ".concat(toString(elm), " for event \"").concat(type, "\". Expected an EventListener but received ").concat(listener, ".")); // TODO: issue #420
        // this is triggered when the component author attempts to add a listener programmatically into a lighting element node

        if (!isUndefined(options)) {
          assert$1.logWarning("The 'addEventListener' method in 'LightningElement' does not support more than 2 arguments. Options to make the listener passive, once, or capture are not allowed but received: ".concat(toString(options)), elm);
        }
      }
    ...
    function addShadowRootEventListener(sr, type, listener, options) {
      {
        assert$1.invariant(isFunction(listener), "Invalid second argument for this.template.addEventListener() in ".concat(toString(sr), " for event \"").concat(type, "\". Expected an EventListener but received ").concat(listener, ".")); // TODO: issue #420
        // this is triggered when the component author attempts to add a listener programmatically into its Component's shadow root

        if (!isUndefined(options)) {
          assert$1.logWarning("The 'addEventListener' method in 'ShadowRoot' does not support more than 2 arguments. Options to make the listener passive, once, or capture are not allowed but received: ".concat(toString(options)), getHost(sr));
        }
      }
    ...
    function handleFocus(elm) {
      {
        assert$1.invariant(isDelegatingFocus(elm), "Invalid attempt to handle focus event for ".concat(toString(elm), ". ").concat(toString(elm), " should have delegates focus true, but is not delegating focus"));
    ...
    function handleFocusIn(elm) {
      {
        assert$1.invariant(tabIndexGetter.call(elm) === -1, "Invalid attempt to handle focus in  ".concat(toString(elm), ". ").concat(toString(elm), " should have tabIndex -1, but has tabIndex ").concat(tabIndexGetter.call(elm)));
    ...
    function ignoreFocusIn(elm) {
      {
        assert$1.invariant(tabIndexGetter.call(elm) !== -1, "Invalid attempt to ignore focus in  ".concat(toString(elm), ". ").concat(toString(elm), " should not have tabIndex -1"));
      }
      ...
      return {
        appendChild: {
          value: function value(aChild) {
            if (_instanceof(this, Element) && options.isPortal !== true) {
      ...
      assign(descriptors, {
        ...
        addEventListener: {
          value: function value(type) {
            assert$1.invariant(!isRendering, "".concat(vmBeingRendered, ".render() method has side effects on the state of ").concat(toString(sr), " by adding an event listener for \"").concat(type, "\".")); // Typescript does not like it when you treat the `arguments` object as an array
            // @ts-ignore type-mismatch
        ...
        querySelector: {
          value: function value() {
            ...
            assert$1.isFalse(isBeingConstructed(vm), "this.template.querySelector() cannot be called during the construction of the custom element for ".concat(vm, " because no content has been rendered yet.")); // Typescript does not like it when you treat the `arguments` object as an array
            // @ts-ignore type-mismatch
        ...
        querySelectorAll: {
          value: function value() {
            var vm = getShadowRootVM(this);
            assert$1.isFalse(isBeingConstructed(vm), "this.template.querySelectorAll() cannot be called during the construction of the custom element for ".concat(vm, " because no content has been rendered yet.")); // Typescript does not like it when you treat the `arguments` object as an array
            // @ts-ignore type-mismatch
    ...
    function assertAttributeReflectionCapability(vm, attrName) {
      ...
      if (!isUndefined(getNodeOwnerKey$1(elm)) && isAttributeLocked(elm, attrName) && propsConfig && propName && propsConfig[propName]) {
        assert$1.logError("Invalid attribute \"".concat(StringToLowerCase.call(attrName), "\" for ").concat(vm, ". Instead access the public property with `element.").concat(propName, ";`."), elm);
      }
    }
    ...
    function assertAttributeMutationCapability(vm, attrName) {
      ...
      if (!isUndefined(getNodeOwnerKey$1(elm)) && isAttributeLocked(elm, attrName)) {
        assert$1.logError("Invalid operation on Element ".concat(vm, ". Elements created via a template should not be mutated using DOM APIs. Instead of attempting to update this element directly to change the value of attribute \"").concat(attrName, "\", you can update the state of the component, and let the engine to rehydrate the element accordingly."), elm);
      }
    }
    ...
    function getCustomElementRestrictionsDescriptors(elm, options) {
      ...
      return assign(descriptors, {
        ...
        addEventListener: {
          value: function value(type) {
            assert$1.invariant(!isRendering, "".concat(vmBeingRendered, ".render() method has side effects on the state of ").concat(toString(elm), " by adding an event listener for \"").concat(type, "\".")); // Typescript does not like it when you treat the `arguments` object as an array
            // @ts-ignore type-mismatch
        ...
        setAttribute: {
          value: function value(attrName, _value) {
            ...
            if (isString(attrName)) {
              var _propName5 = getPropNameFromAttrName(attrName);

              var info = getGlobalHTMLPropertiesInfo();

              if (info[_propName5] && info[_propName5].attribute) {
                var _info$_propName = info[_propName5],
                    error = _info$_propName.error,
                    experimental = _info$_propName.experimental;

                if (error) {
                  assert$1.logError(error, getComponentVM(this).elm);
                } else if (experimental) {
                  assert$1.logError("Attribute `".concat(attrName, "` is an experimental attribute that is not standardized or supported by all browsers. Property \"").concat(_propName5, "\" and attribute \"").concat(attrName, "\" are ignored."), getComponentVM(this).elm);
                }
              }
            } // Typescript does not like it when you treat the `arguments` object as an array
            // @ts-ignore type-mismatch
        ...
        descriptors[propName] = {
          get: function get() {
            var _info$propName = info[propName],
                ...
                attribute = _info$propName.attribute,
                readOnly = _info$propName.readOnly,
    ...
    function createElmHook(vnode) {
...

      {
        ...
        var _isPortal = !isUndefined(_context) && !isUndefined(_context.lwc) && _context.lwc.dom === LWCDOMMode.manual;
    ...
    function register(service) {
      {
        assert$1.isTrue(isObject(service), "Invalid service declaration, ".concat(service, ": service must be an object"));
      }
    ...
    function h(sel, data, children) {
      {
        ...
        if (data.style && !isString(data.style)) {
          assert$1.logWarning("Invalid 'style' attribute passed to <".concat(sel, "> should be a string value, and will be ignored."), vmBeingRendered.elm);
        }
    ...
    function ti(value) {
      ...
      {
        if (shouldNormalize) {
          assert$1.logWarning("Invalid tabindex value `".concat(toString(value), "` in template for ").concat(vmBeingRendered, ". This attribute can only be set to 0 or -1."), vmBeingRendered.elm);
        }
      }
    ...
    function i(iterable, factory) {
      ...
      if (isUndefined(iterable) || iterable === null) {
        {
          assert$1.logWarning("Invalid template iteration for value \"".concat(iterable, "\" in ").concat(vmBeingRendered, ", it should be an Array or an iterable Object."), vmBeingRendered.elm);
        }
      ...
      {
        assert$1.isFalse(isUndefined(iterable[SymbolIterator]), "Invalid template iteration for value `".concat(iterable, "` in ").concat(vmBeingRendered, ", it requires an array-like object, not `null` or `undefined`."));
      }

      var iterator = iterable[SymbolIterator]();

      {
        assert$1.isTrue(iterator && isFunction(iterator.next), "Invalid iterator function for \"".concat(iterable, "\" in ").concat(vmBeingRendered, "."));
      }
    ...
    function k(compilerKey, obj) {
      switch (_typeof(obj)) {
        ...
        case 'object':
          {
            assert$1.fail("Invalid key value \"".concat(obj, "\" in ").concat(vmBeingRendered, ". Key must be a string or number."));
          }
    ...
    function gid(id) {
      if (isUndefined(id) || id === '') {
        {
          assert$1.logError("Invalid id value \"".concat(id, "\". Expected a non-empty string."), vmBeingRendered.elm);
        }
    ...
    function validateSlots(vm, html) {
      ...
      for (var slotName in cmpSlots) {
        ...
        assert$1.isTrue(isArray(cmpSlots[slotName]), "Slots can only be set to an array, instead received ".concat(toString(cmpSlots[slotName]), " for slot \"").concat(slotName, "\" in ").concat(vm, "."));

        if (ArrayIndexOf.call(slots, slotName) === -1) {
          // TODO: this should never really happen because the compiler should always validate
          // tslint:disable-next-line no-production-assert
          assert$1.logWarning("Ignoring unknown provided slot name \"".concat(slotName, "\" in ").concat(vm, ". This is probably a typo on the slot attribute."), vm.elm);
        }
      }
    }
    ...
    function validateFields(vm, html) {
      ...
      forEach.call(ids, function (propName) {
        if (!(propName in component)) {
          // tslint:disable-next-line no-production-assert
          assert$1.logWarning("The template rendered by ".concat(vm, " references `this.").concat(propName, "`, which is not declared. This is likely a typo in the template."), vm.elm);
        } else if (hasOwnProperty$1.call(component, propName)) {
          // tslint:disable-next-line no-production-assert
          assert$1.fail("".concat(component, "'s template is accessing `this.").concat(toString(propName), "`, which is considered a non-reactive private field. Instead access it via a getter or make it reactive by decorating it with `@track ").concat(toString(propName), "`."));
        }
      });
    }
    ...
    function evaluateTemplate(vm, html) {
      {
        assert$1.isTrue(vm && "cmpRoot" in vm, "".concat(vm, " is not a vm."));
        assert$1.isTrue(isFunction(html), "evaluateTemplate() second argument must be an imported template instead of ".concat(toString(html)));
      ...
      {
        assert$1.isTrue(isObject(context.tplCache), "vm.context.tplCache must be an object associated to ".concat(cmpTemplate, ".")); // validating slots in every rendering since the allocated content might change over time
    ...
    function invokeEventListener(vm, fn, thisValue, event) {
      ...
      try {
        {
          assert$1.isTrue(isFunction(fn), "Invalid event handler for event '".concat(event.type, "' on ").concat(vm, "."));
        }
    ...
    function renderComponent(vm) {
      ...
      {
        assert$1.invariant(isArray(vnodes), "".concat(vm, ".render() should always return an array of vnodes instead of ").concat(vnodes));
      }
    ...
    function markComponentAsDirty(vm) {
      {
        ...
        assert$1.isFalse(isRendering, "markComponentAsDirty() for ".concat(vm, " cannot be called during rendering of ").concat(vmBeingRendered, "."));
      }
    ...
    function getComponentAsString(component) {
      ...
      return "<".concat(StringToLowerCase.call(tagNameGetter.call(vm.elm)), ">");
    }
    /*
     * Copyright (c) 2018, salesforce.com, inc.
     * All rights reserved.
    ...
    function createBridgeToElementDescriptor(propName, descriptor) {
      ...
      return {
        ...
        get: function get() {
          ...
          if (isBeingConstructed(vm)) {
            {
              assert$1.logError("".concat(vm, " constructor should not read the value of property \"").concat(propName, "\". The owner component has not yet set the value. Instead use the constructor to set default values for properties."), vm.elm);
            }
        ...
        set: function set(newValue) {
          ...
          {
            assert$1.isTrue(vm && "cmpRoot" in vm, "".concat(vm, " is not a vm."));
            assert$1.invariant(!isRendering, "".concat(vmBeingRendered, ".render() method has side effects on the state of ").concat(vm, ".").concat(propName));
            assert$1.isFalse(isBeingConstructed(vm), "Failed to construct '".concat(getComponentAsString(this), "': The result must not have attributes."));
            assert$1.invariant(!isObject(newValue) || isNull(newValue), "Invalid value \"".concat(newValue, "\" for \"").concat(propName, "\" of ").concat(vm, ". Value cannot be an object, must be a primitive value."));
          }
    ...
    function BaseLightningElement() {
      ...
      {
        assert$1.isTrue(vmBeingConstructed && "cmpRoot" in vmBeingConstructed, "".concat(vmBeingConstructed, " is not a vm."));
        assert$1.invariant(_instanceof(vmBeingConstructed.elm, HTMLElement), "Component creation requires a DOM element to be associated to ".concat(vmBeingConstructed, "."));
      }
    ...
    BaseLightningElement.prototype = {
      ...
      dispatchEvent: function dispatchEvent(event) {
...

        {
          if (arguments.length === 0) {
            throw new Error("Failed to execute 'dispatchEvent' on ".concat(getComponentAsString(this), ": 1 argument required, but only 0 present."));
          }

          if (!_instanceof(event, GlobalEvent)) {
            throw new Error("Failed to execute 'dispatchEvent' on ".concat(getComponentAsString(this), ": parameter 1 is not of type 'Event'."));
          }

          var evtName = event.type;
          assert$1.isFalse(isBeingConstructed(vm), "this.dispatchEvent() should not be called during the construction of the custom element for ".concat(getComponentAsString(this), " because no one is listening for the event \"").concat(evtName, "\" just yet."));

          if (vm.idx === 0) {
            assert$1.logWarning("Unreachable event \"".concat(evtName, "\" dispatched from disconnected element ").concat(getComponentAsString(this), ". Events can only reach the parent element after the element is connected (via connectedCallback) and before the element is disconnected(via disconnectedCallback)."), elm);
          }

          if (!evtName.match(/^[a-z]+([a-z0-9]+)?$/)) {
            assert$1.logWarning("Invalid event type \"".concat(evtName, "\" dispatched in element ").concat(getComponentAsString(this), ". Event name should only contain lowercase alphanumeric characters."), elm);
          }
        }
      ...
      addEventListener: function addEventListener(type, listener, options) {
        ...
        {
          assert$1.isTrue(vm && "cmpRoot" in vm, "".concat(vm, " is not a vm."));
          assert$1.invariant(!isRendering, "".concat(vmBeingRendered, ".render() method has side effects on the state of ").concat(vm, " by adding an event listener for \"").concat(type, "\"."));
          assert$1.invariant(isFunction(listener), "Invalid second argument for this.addEventListener() in ".concat(vm, " for event \"").concat(type, "\". Expected an EventListener but received ").concat(listener, "."));
        }
      ...
      setAttributeNS: function setAttributeNS(ns, attrName, value) {
        ...
        {
          assert$1.isFalse(isBeingConstructed(getComponentVM(this)), "Failed to construct '".concat(getComponentAsString(this), "': The result must not have attributes."));
        }
      ...
      getBoundingClientRect: function getBoundingClientRect() {
        ...
        {
          var vm = getComponentVM(this);
          assert$1.isFalse(isBeingConstructed(vm), "this.getBoundingClientRect() should not be called during the construction of the custom element for ".concat(getComponentAsString(this), " because the element is not yet in the DOM, instead, you can use it in one of the available life-cycle hooks."));
        }
      ...
      querySelector: function querySelector(selectors) {
        ...
        {
          assert$1.isFalse(isBeingConstructed(vm), "this.querySelector() cannot be called during the construction of the custom element for ".concat(getComponentAsString(this), " because no children has been added to this element yet."));
        }
      ...
      querySelectorAll: function querySelectorAll(selectors) {
        ...
        {
          assert$1.isFalse(isBeingConstructed(vm), "this.querySelectorAll() cannot be called during the construction of the custom element for ".concat(getComponentAsString(this), " because no children has been added to this element yet."));
        }
      ...
      getElementsByTagName: function getElementsByTagName(tagNameOrWildCard) {
        ...
        {
          assert$1.isFalse(isBeingConstructed(vm), "this.getElementsByTagName() cannot be called during the construction of the custom element for ".concat(getComponentAsString(this), " because no children has been added to this element yet."));
        }
      ...
      getElementsByClassName: function getElementsByClassName(names) {
        ...
        {
          assert$1.isFalse(isBeingConstructed(vm), "this.getElementsByClassName() cannot be called during the construction of the custom element for ".concat(getComponentAsString(this), " because no children has been added to this element yet."));
        }
    ...
    function createComponentDef(Ctor, meta, subclassComponentName) {
      {
        assert$1.isTrue(isElementComponent(Ctor, subclassComponentName), "".concat(Ctor, " is not a valid component, or does not extends LightningElement from \"lwc\". You probably forgot to add the extend clause on the class declaration.")); // local to dev block
    ...
    function addInsertionIndex(vm) {
      {
        assert$1.isTrue(vm && "cmpRoot" in vm, "".concat(vm, " is not a vm."));
        assert$1.invariant(vm.idx === 0, "".concat(vm, " is already locked to a previously generated idx."));
      }
    ...
    function removeInsertionIndex(vm) {
      {
        assert$1.isTrue(vm && "cmpRoot" in vm, "".concat(vm, " is not a vm."));
        assert$1.invariant(vm.idx > 0, "".concat(vm, " is not locked to a previously generated idx."));
      }
    ...
    function createVM(tagName, elm, Ctor, options) {
      {
        assert$1.invariant(_instanceof(elm, HTMLElement), "VM creation requires a DOM element instead of ".concat(elm, "."));
      }
    ...
    function rehydrate(vm) {
      {
        assert$1.isTrue(vm && "cmpRoot" in vm, "".concat(vm, " is not a vm."));
        assert$1.isTrue(_instanceof(vm.elm, HTMLElement), "rehydration can only happen after ".concat(vm, " was patched the first time."));
      }
    ...
    function patchErrorBoundaryVm(errorBoundaryVm) {
      {
        assert$1.isTrue(errorBoundaryVm && "component" in errorBoundaryVm, "".concat(errorBoundaryVm, " is not a vm."));
        assert$1.isTrue(_instanceof(errorBoundaryVm.elm, HTMLElement), "rehydration can only happen after ".concat(errorBoundaryVm, " was patched the first time."));
    ...
    function destroyChildren(children) {
      for (var _i28 = 0, _len24 = children.length; _i28 < _len24; _i28 += 1) {
        ...
        try {
          ...
          {
            var vm = getCustomElementVM(elm);
            assert$1.logError("Internal Error: Failed to disconnect component ".concat(vm, ". ").concat(e), elm);
          }
        }
      }
    ...
    function getHostElement(elm) {
      {
        ...
        assert$1.isTrue(isNull(parentElementGetter.call(elm)), "getHostElement should only be called if the parent element of ".concat(elm, " is null"));
      }
        ...
        value: function addEventListener(type, listener) {
          ...
          switch (type) {
            case CONNECT:
              ...
              {
                __callKey2(assert$3, "isFalse", __callKey1(connectedListeners, "includes", listener), 'must not call addEventListener("connect") with the same listener');
              }
            ...
            case DISCONNECT:
              ...
              {
                __callKey2(assert$3, "isFalse", __callKey1(disconnectedListeners, "includes", listener), 'must not call addEventListener("disconnect") with the same listener');
              }
    ...
    var wireService = {
      wiring: function wiring(cmp, data, def, context) {
        ...
        var _loop = function _loop(i, len) {
          ...
          {
            __callKey2(assert$3, "isTrue", wireDef._ES5ProxyType ? wireDef.get("adapter") : wireDef.adapter, __concat("@wire on \"", wireTarget, "\": adapter id must be truthy"));

            __callKey2(assert$3, "isTrue", adapterFactory, __concat(__concat("@wire on \"", wireTarget, "\": unknown adapter id: "), String(wireDef._ES5ProxyType ? wireDef.get("adapter") : wireDef.adapter))); // enforce restrictions of reactive parameters
            ...
            if (wireDef._ES5ProxyType ? wireDef.get("params") : wireDef.params) {
              __callKey1(Object.compatKeys(wireDef._ES5ProxyType ? wireDef.get("params") : wireDef.params), "forEach", function (param) {
                ...
                __callKey1(segments, "forEach", function (segment) {
                  __callKey2(assert$3, "isTrue", (segment._ES5ProxyType ? segment.get("length") : segment.length) > 0, __concat("@wire on \"", wireTarget, "\": reactive parameters must not be empty"));
                });

                __callKey2(assert$3, "isTrue", (segments._ES5ProxyType ? segments.get(0) : segments[0]) !== wireTarget, __concat(__concat("@wire on \"", wireTarget, "\": reactive parameter \""), segments._ES5ProxyType ? segments.get(0) : segments[0], "\" must not refer to self")); // restriction for dot-notation reactive parameters
    ...
    function register$2(adapterId, adapterFactory) {
      {
        __callKey2(assert$3, "isTrue", adapterId, 'adapter id must be truthy');

        __callKey2(assert$3, "isTrue", typeof adapterFactory === 'function', 'adapter factory must be a callable');
      }
...
}(Proxy.callKey0,Proxy.setKey,Proxy.callKey1,Proxy.callKey2,Proxy.concat,TalonCompat.babel.helpers.slicedToArray,TalonCompat.babel.helpers.assertThisInitialized,TalonCompat.babel.helpers.get,TalonCompat.babel.helpers.possibleConstructorReturn,TalonCompat.babel.helpers.getPrototypeOf,TalonCompat.babel.helpers.inherits,TalonCompat.babel.helpers.defineProperty,TalonCompat.babel.helpers.classCallCheck,TalonCompat.babel.helpers.createClass,TalonCompat.babel.helpers.instanceof,TalonCompat.babel.helpers.typeof,Proxy.inKey,Proxy.callKey3,Proxy.callKey4,TalonCompat.babel.helpers.toArray,TalonCompat.babel.regenerator,TalonCompat.babel.helpers.asyncToGenerator,Proxy.deleteKey,Proxy.hasOwnProperty,TalonCompat.babel.helpers.toConsumableArray,TalonCompat.babel.helpers.objectDestructuringEmpty,Proxy.setKeyPostfixDecrement,Proxy.setKeyPostfixIncrement));
